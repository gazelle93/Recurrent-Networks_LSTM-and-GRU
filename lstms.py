...
class rnncell(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        ...
        self.update_gate_input = nn.Linear(input_dim, hidden_dim)
        self.update_gate_hidden = nn.Linear(input_dim, hidden_dim)
...
class lstmcell(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        ...
        self.input_gate_input = nn.Linear(input_dim, hidden_dim)
        self.input_gate_hidden = nn.Linear(input_dim, hidden_dim)
        
        # forget gate weight & bias
        self.forget_gate_input = nn.Linear(input_dim, hidden_dim)
        self.forget_gate_hidden = nn.Linear(input_dim, hidden_dim)
        
        # cell gate weight & bias
        self.cell_gate_input = nn.Linear(input_dim, hidden_dim)
        self.cell_gate_hidden = nn.Linear(input_dim, hidden_dim)
        
        # output gate weight & bias
        self.output_gate_input = nn.Linear(input_dim, hidden_dim)
        self.output_gate_hidden = nn.Linear(input_dim, hidden_dim)
...
class grucell(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        ...
        self.reset_gate_input = nn.Linear(input_dim, hidden_dim)
        self.reset_gate_hidden = nn.Linear(input_dim, hidden_dim)
        ...
        # output gate weight & bias
        self.new_gate_input = nn.Linear(input_dim, hidden_dim)
        self.new_gate_hidden = nn.Linear(input_dim, hidden_dim)
...
class lstmlayer(nn.Module):
    def __init__(self, input_dim, hidden_dim, reverse=False, drouput_rate=0.1, sequential_model="lstm"):
        ...
        if sequential_model == "lstm":
            if reverse:
                self.reverselayer = lstmcell(input_dim, hidden_dim)
            else:
                self.forwardlayer = lstmcell(input_dim, hidden_dim)
                
        elif sequential_model == "gru":
            if reverse:
                self.reverselayer = grucell(input_dim, hidden_dim)
            else:
                self.forwardlayer = grucell(input_dim, hidden_dim)

        self.dropout = nn.Dropout(p=drouput_rate)
    
    def forward(self, input_embedding, is_dropout=True):
        output_list = torch.zeros((input_embedding.size()))
        cell_list = torch.zeros((input_embedding.size()))
        
        previous_cell_states = torch.zeros((input_embedding.size()[0], self.hidden_dim))
        hidden_states = torch.zeros((input_embedding.size()[0], self.hidden_dim))
...
class LSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim, bidirection=False, sequential_model="lstm"):
        super(LSTM, self).__init__()
        self.bidirection = bidirection
        
        self.forwardlstmlayer = lstmlayer(input_dim, hidden_dim, sequential_model=sequential_model)
        
        if bidirection:
            self.reverselstmlayer = lstmlayer(input_dim, hidden_dim, bidirection, sequential_model=sequential_model)
    
    def forward(self, input_embedding, is_dropout=False):
        forward_output, forward_cell = self.forwardlstmlayer(input_embedding=input_embedding,
                                                             is_dropout=is_dropout)
        
        if self.bidirection:
            reverse_output, reverse_cell = self.reverselstmlayer(input_embedding=input_embedding,
                                                                 is_dropout=is_dropout)
            bilstm_output = torch.cat((forward_output, reverse_output), dim=2)
            bilstm_cell = torch.cat((forward_cell, reverse_cell), dim=2)
            return bilstm_output, bilstm_cell
